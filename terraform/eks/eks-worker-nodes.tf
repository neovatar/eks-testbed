...
resource "aws_iam_role_policy_attachment" "eks-test-node-AmazonEKSWorkerNodePolicy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
  role       = "${aws_iam_role.eks-test-node.name}"
}

resource "aws_iam_role_policy_attachment" "eks-test-node-AmazonEKS_CNI_Policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
  role       = "${aws_iam_role.eks-test-node.name}"
}

resource "aws_iam_role_policy_attachment" "eks-test-node-AmazonEC2ContainerRegistryReadOnly" {
...
locals {
  eks-test-node-userdata = <<USERDATA
#!/bin/bash
set -o xtrace
/etc/eks/bootstrap.sh --apiserver-endpoint '${aws_eks_cluster.eks-test.endpoint}' --b64-cluster-ca '${aws_eks_cluster.eks-test.certificate_authority.0.data}' '${var.cluster-name}'
USERDATA
}

resource "aws_launch_configuration" "eks-test" {
  ...
  user_data_base64            = "${base64encode(local.eks-test-node-userdata)}"
...
resource "aws_autoscaling_group" "eks-test" {
  desired_capacity     = 2
  launch_configuration = "${aws_launch_configuration.eks-test.id}"
  max_size             = 2
  min_size             = 1
  name                 = "eks-test"
